...
void PatClass::LossComputeErrorImage(Eigen::Matrix<float, Eigen::Dynamic, 1>* patdest, Eigen::Matrix<float, Eigen::Dynamic, 1>* wdest, const Eigen::Matrix<float, Eigen::Dynamic, 1>* patin,  const Eigen::Matrix<float, Eigen::Dynamic, 1>*  tmpin)
{
  ...
  if (op->costfct==0) // L2 cost function
  {
    ...
    {
      const __m128 mpd = _mm_sub_ps(_mm_load_ps(pa), _mm_load_ps(te)); // difference image
#if defined(USE_SSE)
      const __m128 mpw = _mm_andnot_ps(op->negzero, mpd);
#elif defined(USE_NEON)
      const float32x4_t mpw = vabsq_f32(mpd);
#endif
      _mm_store_ps(pd, mpd);
      _mm_store_ps(pw, mpw);
    }
  ...
  else if (op->costfct==1) // L1 cost function
  {
    for (int i = op->novals / 4; i--; pd += 4, pa += 4, te += 4, pw += 4)
    {
      __m128 mpd = _mm_sub_ps(_mm_load_ps(pa), _mm_load_ps(te));   // difference image
#if defined(USE_SSE)
      const __m128 mpw = _mm_sqrt_ps(_mm_andnot_ps(op->negzero, mpd));
      mpd = _mm_or_ps(_mm_and_ps(op->negzero, mpd), mpw);  // sign(pdiff) * sqrt(abs(pdiff))
#elif defined(USE_NEON)
      const float32x4_t mpw = vsqrtq_f32(vabsq_f32(mpd));;
  ...
  else if (op->costfct==2) // Pseudo Huber cost function
  {
    ...
    {
      __m128 mpd = _mm_sub_ps(_mm_load_ps(pa), _mm_load_ps(te));   // difference image
      const __m128 mpw = _mm_sqrt_ps(_mm_mul_ps(_mm_sub_ps(_mm_sqrt_ps(_mm_add_ps(op->ones, _mm_div_ps(_mm_mul_ps(mpd, mpd), op->normoutlier_tmpbsq))), op->ones), op->normoutlier_tmp2bsq));
#if defined(USE_SSE)
